...
""" Fork from falsk_script。

用法参考：http://flask-script.readthedocs.io/en/latest/
"""
...
from .commands import Group, Option, Command, Shell
...
def add_help(parser, help_args): 
    if not help_args:
        return
    parser.add_argument(*help_args, action='help', default=argparse.SUPPRESS, help='show this help message and exit')
...
class Manager(object):
    ...
    :param app: Sanic instance, or callable returning a Sanic instance.
    ...
    def handle(self, prog, args=None):
        ...
        for handle in func_stack:
            ...
            config = dict((k, v) for k, v in kwargs.items() if k in config_keys)

            # remove application config keys from handle kwargs
            kwargs = dict((k, v) for k, v in kwargs.items() if k not in config_keys)
